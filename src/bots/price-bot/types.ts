...
export interface AutoTradeParams {
  ...
  /**
   * 买入前摇筛子：面数 N（0/1 = 不启用，6 = 标准六面筛）。
   *
   * 玩法（不放回抽样）：所有闸门都过了、真要下单的那一刻摇一次筛子，
   * 摇到最大面（=N）才真下单；没中就把这一面从池子里拿掉、这次机会作废，
   * 下次同一盘口再触发买入信号时从剩下的面里摇。池子越摇越小，
   * 到第 N 次时只剩最大面，必中——所以「最多摇 N 次就一定下单」。
   *
   * 等价形式：最大面在一个随机排列里的位置 K ~ 均匀分布{1..N}，
   * 即「第 K 次买入机会才下单」。每次摇中的概率是 1/剩余面数
   * （1/N, 1/(N−1), … , 1/1），但**无条件**看每一次机会都是 1/N。
   *
   * ⚠ 实测代价很大，所以默认关闭。回测（9.25，n=61 条 outcome 已知的规则）：
   *   基线（不摇筛，第 1 次机会即买）  61 单  胜率 91.8%  净利 $18.80  ROI +7.2%
   *   6 面筛                          13.1 单 胜率 87.2%  净利 $0.18   ROI +0.3%
   * 原因有两层。一是**机会数不够**：86 条规则里 68 条一辈子只产生 1 次买入
   * 机会（中位数 1，均值 2.43），所以六面筛对它们不是「延迟买入」而是
   * 直接 5/6 概率永久放弃，只有 12.8% 的盘口能攒够 6 次机会做到必买。
   * 二是**它挑的方向是反的**：筛子偏向留下反复触发的盘口，而反复触发的
   * 恰恰是假突破（4 条多机会规则里 3 条净亏），一次性触发的干净信号反被丢掉。
   *
   * 想温和一点就把面数调小（2 面筛保住 41% 的净利、3 面筛 22%）。
   * 想压资金占用，`minMatchMinute` 是有信息的筛选器，筛子是无信息的随机丢弃。
   */
  buyDiceSides?: number
}
...
export const DEFAULT_AUTO_TRADE: Required<AutoTradeParams> = {
  ...
  minMatchMinute: 0,
  makerTickOffset: 1,
  // 0 = 不摇筛子。回测里六面筛把净利从 $18.80 打到 $0.18（见字段注释），
  // 属于代价明确的随机丢弃，绝不能默认打开。
  buyDiceSides: 0,
}
...
export interface PriceMonitorState {
  ...
  /**
   * 买入筛子的剩余面（不放回，见 AutoTradeParams.buyDiceSides）。
   *
   * undefined = 还没摇过，第一次摇时按当时的面数初始化。摇中后清空，
   * 下一笔（maxOrdersPerRule 允许时）重新拿一副完整的筛子。
   *
   * 只活在内存里：进程重启会重新装满池子，等于把已经摇掉的面还回去，
   * 于是重启后再摇中的概率变低。规则寿命就一场比赛（约 2 小时），
   * 不值得为此加一列，但排查「为什么重启后一直没买」时要知道这件事。
   */
  buyDiceFaces?: number[]
}
